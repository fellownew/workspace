...
public class FileTest {
	public static void main(String[] args) {
		...
		File dir = new File("c:\\JAVA");//directory 객체 - 절대경로
...
//	정보조회 메소드들
		...
		String path1 = dir.getPath(); //dir의 경로 가지고 오기
		System.out.println("dir.getPath() : "+path1);
		String path2 = file.getPath(); // file의 경로 가지고 오기
		System.out.println("file.getPath() : "+path2);
		//2. 절대경로를 알려주는메소드
		String absPath1 = file.getAbsolutePath();
		System.out.println("file의 절대경로 : "+absPath1); //이클립스는 이클립스를 기준으로 함.
		
		String absPath2 = dir.getAbsolutePath();
		System.out.println("dir의 절대 경로 : "+absPath2);
		
//	파일이나 디렉토리 존재 여부 체크
		boolean flag = file.exists(); // file이 있는지 확인
		System.out.println(flag);
		flag = dir.exists(); //dir이 있는지 확인
		System.out.println(flag);
...
//	읽기가능/쓰기가능/실행가능 한 파일/디렉토리인지 체크
		System.out.println("file2 읽기 가능?"+file2.canRead());//읽기 가능?
		System.out.println("file2 쓰기 가능?"+file2.canWrite());//쓰기 가능?
...
//	수정
		...
		//파일 생성
		File file3 = new File(dir2, "newFile.txt");
		try {
